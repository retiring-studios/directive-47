...
/// desktop tests in <c>D47.Panel.Tests</c>, which skip on a machine somebody is
/// using: those can run anywhere nobody is typing, including CI, so skipping
/// costs nothing. These run in exactly one place — the dev PC, with the game up
/// on a second monitor — and there is nowhere else for them to be run later. A
/// skip there is a green build that checked nothing.
/// </para>
///
/// <para>
